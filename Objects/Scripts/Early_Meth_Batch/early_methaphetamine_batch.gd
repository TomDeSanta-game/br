...
func _ready() -> void:
    ...
    randomize()
    # Slightly adjust colors to make each crystal unique
...
func _on_body_entered(body: Node2D) -> void:
    if body == player:
        ...
        # Animate collection with a quick scale effect and color change
...
func _on_glow_timer_timeout() -> void:
    ...
    # Occasionally emit sparkles
    if randf() > 0.7:
        sparkles.emitting = true

func animate_crystal_glow() -> void:
    # Choose a random crystal to glow
