...
func _input(event):
	# Check for multiple possible input mappings, ensure at least one works
	if event.is_action_pressed("ui_home") or event.is_action_pressed("INTERACT") or event.is_action_pressed("Interact"):
