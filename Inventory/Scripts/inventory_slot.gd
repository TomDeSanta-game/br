...
func _ready():
	...
	# Apply rounded corners to the slot
...
func set_item(data):
	...
	if item_data:
		...
		# Setup rarity indicator if applicable
		if item_data.has("rarity") and RARITY_COLORS.has(item_data.rarity):
			...
			# Add a subtle glow for higher rarities
		...
		# Update background color for filled slot
		var stylebox = get_theme_stylebox("panel").duplicate()
		stylebox.bg_color = SLOT_COLOR_FILLED
		add_theme_stylebox_override("panel", stylebox)
		
		# Add a subtle zoom-in animation
...
func clear_slot():
	...
	# Update background color for empty slot
	var stylebox = get_theme_stylebox("panel").duplicate()
	stylebox.bg_color = SLOT_COLOR_EMPTY
	add_theme_stylebox_override("panel", stylebox)
...
func select():
	...
	# Scale up the slot slightly when selected
	var tween = create_tween()
...
func deselect():
	...
	# Reset background color based on item state
	var stylebox = get_theme_stylebox("panel").duplicate()
	if item_data:
		stylebox.bg_color = SLOT_COLOR_FILLED
	else:
